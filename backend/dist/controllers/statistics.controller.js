...
const saveStatsToDb = async (userId, statsObj) => {
    await statistics_model_1.default.findOneAndUpdate({ userId }, { $set: statsObj }, { upsert: true, new: true });
};
...
const calculateTimeBasedStats = async (userId) => {
    ...
    const thisWeekQuizzes = await quiz_model_1.default.countDocuments({
        createdBy: userId,
        createdAt: { $gte: oneWeekAgo }
    });
    const thisMonthQuizzes = await quiz_model_1.default.countDocuments({
        createdBy: userId,
        createdAt: { $gte: oneMonthAgo }
    });
    const last7DaysAttempts = await quizAttempt_model_1.default.find({
        userId,
...
const calculateUserStats = async (userId) => {
    const quizzesCreated = await quiz_model_1.default.countDocuments({ createdBy: userId });

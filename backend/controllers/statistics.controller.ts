...
const getBadges = (points: number) => {
  const badges = [] as { id: string; name: string; description: string; icon: string }[];
...
const saveStatsToDb = async (userId: any, statsObj: any) => {
...
const calculateTimeBasedStats = async (userId: any) => {
  const now = new Date();
  ...
  const dailyScores: { day: string; score: number; target: number; aboveTarget: boolean }[] = [];
...
export const getUserStats = async (req: Request, res: Response): Promise<void> => {
  try {
    const userId = (req.user as any)?._id || req.userId || req.params.userId;
